/**
 * @file VNEQTS_tuned.cpp
 * @brief Parameterized VNEQTS experiment; not a reproduction of paper Table III.
 *
 * The program distributes a fixed budget of atomic compute units (ACUs) among
 * candidate base-station sites. Candidate deployments are evaluated using
 * coverage, served workload, load balance, and energy efficiency. Workloads
 * that cannot be served within the distance and capacity limits fall back to
 * the cloud.
 *
 * The implementation is self-contained and requires a C++17 compiler only.
 */
...
constexpr double OBSERVATION_SECONDS = 15811200.0;  // 183 days.
...
constexpr double D_MAX_METERS = 4800.0;
...
// One candidate ACU deployment and its cached evaluation metrics.
struct Individual {
...
// Deterministic workload assignment produced for one deployment.
...
// Population-diversity values returned by the entropy calculation.
...
vector<int> workload_order;
vector<vector<int>> service_candidates;
double total_workload = 0.0;
double acu_capacity = 0.0;
...
// Returns the great-circle distance in meters between two geographic points.
...
// Precomputes the symmetric all-pairs distance matrix used during evaluation.
void buildDistanceMatrix(const vector<BaseStation>& stations) {
    ...
    workload_order.resize(n);
    iota(workload_order.begin(), workload_order.end(), 0);
    stable_sort(workload_order.begin(), workload_order.end(), [&](int a, int b) {
        return stations[a].workload > stations[b].workload;
    });
    service_candidates.assign(n, {});
    for (int station = 0; station < n; ++station) {
        for (int site = 0; site < n; ++site) {
            if (distance_matrix[site][station] <= static_cast<float>(D_MAX_METERS)) {
                service_candidates[station].push_back(site);
            }
        }
        sort(service_candidates[station].begin(), service_candidates[station].end(),
             [&](int a, int b) {
                 const float da = distance_matrix[a][station];
                 const float db = distance_matrix[b][station];
                 return da != db ? da < db : a < b;
             });
    }
}

// Checks the total ACU budget and every per-site capacity bound.
...
// Assigns base-station workloads to active edge sites.
ServiceMapping computeServiceMapping(
    ...
    vector<double> capacities(n, 0.0);
    for (int i = 0; i < n; ++i) {
        if (deployment[i] > 0) {
            capacities[i] = deployment[i] * acu_capacity;
        }
    }
    for (int station : workload_order) {
        int best_site = -1;
        for (int site : service_candidates[station]) {
            if (deployment[site] > 0 &&
                mapping.site_load[site] + stations[station].workload <=
                    capacities[site] + NUMERIC_EPS) {
                best_site = site;
                break;
            }
        }
...
void calculateFitness(
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (mapping.site_load[i] > NUMERIC_EPS) {
            // Load balance excludes active sites that receive no workload.
            positive_load_utilizations.push_back(utilization);
        }
        energy += count * (1.0 + utilization);
    }
...
// Computes magnitude-aware population entropy and the associated spectrum.
...
EntropyMetrics computeVne(
    ...
    double total_centered_norm2 = 0.0;
    ...
    const int full_sites = ACU_BUDGET / MAX_ACUS_PER_SITE;
    const int remainder = ACU_BUDGET % MAX_ACUS_PER_SITE;
    const double maximum_deployment_norm2 =
        full_sites * MAX_ACUS_PER_SITE * MAX_ACUS_PER_SITE +
        remainder * remainder;
    const double mean_centered_norm2 =
        total_centered_norm2 / static_cast<double>(population_size);
    const double dispersion = maximum_deployment_norm2 > 0.0
        ? sqrt(max(0.0, mean_centered_norm2) /
               maximum_deployment_norm2)
        : 0.0;

    EntropyMetrics metrics;
    metrics.spectral = max(0.0, min(1.0, spectral_entropy));
    metrics.dispersion = max(0.0, min(1.0, dispersion));
    metrics.convergence = metrics.spectral * metrics.dispersion;
    return metrics;
}

// Rebuilds selected non-elite individuals when the search stagnates.
void spectralEscape(
    ...
    // Principal-eigenvector centrality identifies the geometric
    // representatives of the current trap independently of fitness rank.
    ...
    for (int individual_index : rebuild_indices) {
        ...
        // Restore feasibility, then locally refine the distant solution.
...
// Produces a feasible child by uniform site-wise crossover.
...
// Mutates a deployment by moving one ACU between two feasible sites.
void mutate(
    Individual& individual,
    const vector<BaseStation>& stations,
...
// Creates one random feasible deployment with uniform qubit amplitudes.
...
// Reads and validates base-station records from a CSV file.
vector<BaseStation> readBaseStations(const string& filename) {
    ...
    const int duration_column =
        findColumn(header, {"total_duration", "duration", "session_duration"});
    const int workload_column =
        findColumn(header, {"lambda", "lambda_k", "workload"});

    if (id_column < 0 || latitude_column < 0 || longitude_column < 0 ||
        (duration_column < 0 && workload_column < 0)) {
        throw runtime_error(
            "CSV must contain ID, latitude, longitude, and either "
            "total_duration or lambda/workload columns.");
    }
    ...
    while (getline(input, line)) {
        ...
        const int required_index = max(
            ...
            max(duration_column, workload_column));
        ...
        try {
            ...
            if (duration_column >= 0) {
                station.workload =
                    stod(fields[duration_column]) / OBSERVATION_SECONDS;
            } else {
...
void writeDeploymentCsv(
    ...
    const vector<BaseStation>& stations,
    const ServiceMapping& mapping) {
    ofstream output(output_prefix + "_deployment.csv");
    if (!output) {
        throw runtime_error("Cannot create acu_deployment_VNEQTS.csv.");
    }
    output
...
void writeAllocationCsv(
    ...
    const vector<BaseStation>& stations,
    const ServiceMapping& mapping) {
    ofstream output(output_prefix + "_allocation.csv");
    if (!output) {
        throw runtime_error("Cannot create bs_es_allocation_VNEQTS.csv.");
    }
    output
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc != 5) {
            cerr << "Usage: " << argv[0]
                << " <workload.csv> <seed> <generations> <output_prefix>\n";
            return 1;
        }
        ...
        const int max_generations = stoi(argv[3]);

        if (max_generations <= 0) {
            throw invalid_argument(
                "The generation count must be positive.");
        }
        ...
        if (output_prefix.empty()) {
            throw invalid_argument(
                "The output prefix must not be empty.");
        }
        ...
        acu_capacity = ratio * total_workload /
                       static_cast<double>(ACU_BUDGET);
        ...
        ofstream convergence(output_prefix + "_convergence.csv");
        if (!convergence) {
            throw runtime_error("Cannot create convergence_VNEQTS.csv.");
        }
        convergence
            << "generation,fitness,site_coverage,traffic_coverage,"
               "load_balance,energy_efficiency,total_energy,cloud_fallback,"
               "VNE_convergence,VNE_spectral,dispersion,theta_over_pi,"
        ...
        for (int generation = 1;
            ...
            const double drive =
                (1.0 - sensing_entropy.convergence) * eta;
            const double theta =
            ...
            if (inside_escape_window &&
                entropy.convergence < ENTROPY_LOW &&
                ...
                stagnation = 0;
                // Log the diversity actually present after reconstruction.
                entropy = computeVne(
                    population, features, gram, eigenvalues, eigenvectors);
            }
            ...
            if (generation >= 10 && generation % 10 == 0) {
                ...
                cout << "generation=" << setw(4) << generation
                    ...
                    << " fallback_BS=" << global_best.cloud_fallback
                    << '/' << stations.size()
                    << " VNE=" << entropy.convergence
        ...
        for (int i = 0; i < static_cast<int>(stations.size()); ++i) {
            if (global_best.acu_count[i] > 0) {
                max_utilization = max(max_utilization,
                    final_mapping.site_load[i] / (global_best.acu_count[i] * acu_capacity));
            }
        }
        if (!isFeasibleDeployment(global_best.acu_count) ||
            max_utilization > 1.0 + NUMERIC_EPS) {
            throw runtime_error("Final deployment violates capacity or ACU constraints.");
        }
        const double elapsed_seconds = chrono::duration<double>(
            chrono::steady_clock::now() - run_start).count();
        ofstream summary(output_prefix + "_summary.csv");
        if (!summary) {
            throw runtime_error("Cannot create summary CSV.");
        }
        summary << "input,seed,generations,stations,budget,max_acus_per_site,"
                   "ratio,capacity_per_acu,radius_m,fitness,site_coverage,"
                   "traffic_coverage,load_balance,energy_efficiency,total_energy,"
                   "cloud_fallback,active_sites,max_utilization,elapsed_seconds,"
                   "fitness_evaluations,mapping,entropy_control\n"
                << quoted(input_csv) << ',' << seed << ',' << max_generations << ','
                << stations.size() << ',' << ACU_BUDGET << ',' << MAX_ACUS_PER_SITE << ','
                << setprecision(17) << ratio << ',' << acu_capacity << ','
                << D_MAX_METERS << ',' << global_best.fitness << ','
                << global_best.site_coverage << ',' << global_best.traffic_coverage << ','
                << global_best.load_balance << ',' << global_best.energy_efficiency << ','
                << global_best.total_energy << ',' << global_best.cloud_fallback << ','
                << active_sites << ',' << max_utilization << ',' << elapsed_seconds << ','
                << fitness_evaluations << ",capacity-aware,spectral_times_dispersion\n";

        cout << "\nFinal tuned VNEQTS result\n"
